...
const Freelancer = () => {
  ...
  const fetchUserData = async(id) =>{
      axios.get(`http://localhost:5000/fetch-freelancer/${id}`).then(
        (response)=>{
  ...
  const updateUserData = async() =>{
    axios.post(`http://localhost:5000/update-freelancer`, {freelancerId, updateSkills: updateSkills, description: updateDescription}).then(
        (response)=>{
          fetchUserData();
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900 mt-2">Available: <span className="font-bold">₹ {freelancerData.funds}</span></p>
                  </div>
                  {/* optional withdraw placeholder - kept hidden */}
                </div>
              </div>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="text-sm text-gray-600 space-y-3">
                    <div>
                      ...
                      <div className="font-medium text-gray-900">{freelancerData.name || '—'}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Email</div>
                      <div className="font-medium text-gray-900">{freelancerData.email || '—'}</div>
                    </div>
                    <div>
